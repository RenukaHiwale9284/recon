...
public class ReconService
	...
	public Recon getReconForProject(int projectId)
		{
		Recon recon = new Recon() ;
		
		HibernateReader reader = new HibernateReader(projectId);
		
		IncomeDocument as26 = reader.read26ASAggr() ;
				
		IncomeDocument salesRegister  =reader.readSRAggr() ;

		IncomeDocument rtp  =reader.readRPTAggr() ;

		recon.attachPrimarySource(as26) ;	
		recon.attachTdsSource(as26);
			
		recon.attachSource(salesRegister, 2);
		recon.attachSource(rtp, 2);
