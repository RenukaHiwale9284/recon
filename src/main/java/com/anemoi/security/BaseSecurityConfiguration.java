...
public abstract class BaseSecurityConfiguration {
	...
	protected int[] P = { 16, 7, 20, 21, 29, 12, 28, 17, 1, 15, 23, 26, 5,
	...
	protected int[] PC1 = { 57, 49, 41, 33, 25, 17, 9, 1, 58, 50, 42, 34,
	...
	protected   int[] PC2 = { 14, 17, 11, 24, 1, 5, 3, 28, 15, 6, 21, 10,
	...
	protected   int extractBit(byte[] data, int pos) {
		int posByte = pos / 8;
		int posBit = pos % 8;										
		byte tmpB = data[posByte];
	...
	protected   byte[] rotLeft(byte[] input, int len, int pas) {
	...
	protected   byte[] extractBits(byte[] input, int pos, int n) {
	...
	protected   byte[] permutFunc(byte[] input, int[] table) {
	...
	protected   byte[] xor_func(byte[] a, byte[] b) {
	...
	protected  byte[] encrypt64Bloc(byte[] bloc,byte[][] subkeys, boolean isDecrypt) {
	...
	protected   byte[] f_func(byte[] R, byte[] K) {
		byte[] tmp;
	...
	protected   byte[] separateBytes(byte[] in, int len) {
	...
	protected   byte[] concatBits(byte[] a, int aLen, byte[] b, int bLen) {
